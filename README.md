...
| 🎨 **赛博朋克控制台** | NEON OS 深色主题：霓虹辉光、HUD 切角按钮、SVG 矢量图标 |
...
打开 Web UI 后默认进入「📊 面板」，顶部是**快捷操作区**，最常用的两个功能一步直达：

- **⚡ 一键全流程**：建索引 + AI 评分 + 选片推送（推荐日常入口）
- **📡 选片并推送**：选择设备后一键推送到 ZecTrix Note4
...
| 🔄 重试失败 | 重置并重新评分失败的照片 | 出现批量错误后 |

> 任务执行时自动切到面板的实时进度条，支持中途停止。
...
- **规则过滤**：毫秒级，自动标记约 20-30% 低质量照片（截图、连拍等）
...
3. Web UI「🔧 操作」→ 试跑「⏳ 增量评分」，观察实时进度与错误信息
